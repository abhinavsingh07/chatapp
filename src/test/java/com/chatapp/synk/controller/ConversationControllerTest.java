...
class ConversationControllerTest {
    ...
    void testCreate_Success() {
        ...
        // Act
        ResponseEntity<SuccessResponse<ConversationDTO>> response = conversationController.create(mockConversationDTO);
    ...
    void testGetById_WhenConversationFound() {
        ...
        // Act
        ResponseEntity<SuccessResponse<ConversationDTO>> response = conversationController.getById("convo123");
    ...
    void testGetById_WhenConversationNotFound() {
        ...
        // Act
        ResponseEntity<SuccessResponse<ConversationDTO>> response = conversationController.getById("nonexistent");
    ...
    void testGetOrCreateConversation_Success() {
        try (var mockedSecurityUtil = mockStatic(SecurityUtil.class)) {
            ...
            // Arrange
            when(conversationService.getOrCreateConversation(TEST_USER_ID, "user2"))
                    .thenReturn("convo456");
            // Act
            ResponseEntity<SuccessResponse<String>> response = conversationController.getOrCreateConversation("user2");
            ...
            verify(conversationService, times(1)).getOrCreateConversation(TEST_USER_ID, "user2");
        }
    }
    ...
    void testGetUserConversationsLastMessage_WhenConversationsExist() {
        try (var mockedSecurityUtil = mockStatic(SecurityUtil.class)) {
            ...
            when(conversationLastMessageService.findUserConversations(TEST_USER_ID))
                    .thenReturn(List.of(mockLastMsgDTO));

            // Act
            ResponseEntity<SuccessResponse<ConversationLastMsgDTO>> response = conversationController
                    .getUserConversationsLastMessage();
            ...
            assertEquals(1, response.getBody().getData().size());
            verify(conversationLastMessageService, times(1)).findUserConversations(TEST_USER_ID);
        }
    }
